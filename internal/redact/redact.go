...
// than once ("&amp;amp;"). Names that merely end in one of these
// (page_token) are kept. A value runs to the next separator, quote,
// backslash or space, except that a slash a JSON encoder escaped ("\/" or
// "\u002f") stays part of the value, so a token containing "/" is masked
// in full.
...
var signedLinkParam = regexp.MustCompile(
	`(?i)((?:\?|(?:&|\\u0026)(?:amp;|#0*38;|#x0*26;)*)(?:x-amz-signature|x-amz-credential|x-amz-security-token|signature|sig|token|access_token|pwd|password)=)(?:[^&#\s"'\\<>]|\\/|\\u002[fF])*`,
)
