...
func TestGuidanceSeparatesContactOfRecordFromSeatHolder(t *testing.T) {
	...
	for _, want := range []string{
		"Never infer a roster from membership or event data. A membership's key contact is the contact of record, not a seat; a roster row is a seat, not the contact of record — label which one you cite, with its recorded term date where present, never its record stamp (Routing). The committee tools read the LFX v2 committee service. This layer has no committee metric:",
		"this layer carries no committee seats. The membership's contact of record is get_membership_key_contacts; the two can name different people.",
	} {
	...
	const standardWant = `The standard metrics carry no people. Who represents an organization has two answers from two records: the membership's contact of record (get_membership_key_contacts) and the holder of a seat (get_org_committee_seats, search_committee_members). A seat represents the organization when its committee category is Board or its voting_status is Voting Rep or Alternate Voting Rep on any committee — TOC/TSC and the member-class rosters filed under category Other included — never Board alone; Observer, Emeritus and None seats are read from the seat rows, not as representation. A contact carries its updated date; a seat row carries a term date only where recorded, never a tenure date from its record stamps: cite each as recorded on its side, with the date where one is returned, never as "current"; when the two name different people show both side by side, labelled, never merged.`
