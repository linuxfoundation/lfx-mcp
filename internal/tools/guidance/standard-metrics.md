...
| maintainers_by_org | Active maintainers per employer, Linux Foundation projects only (the LF-project filter is built in) | SNAPSHOT | account, active_maintainers | NOT additive; the NULL row is an unresolved employer; no parent-company lens yet, so subsidiaries must be none |
