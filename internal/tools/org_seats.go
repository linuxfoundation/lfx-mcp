...
const membershipContactsNote = "Membership key contacts are the contacts of record for each membership (roles and status as stored, updated_at where one is returned); seats are roster rows; the two can name different people — cite which one you mean."
...
func RegisterGetOrgCommitteeSeats(server *mcp.Server) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Summarise an organization's committee seats as LFX Self Serve's Org Lens Board & Committee tab shows them. " +
			...
			"Returns seats_total, people (distinct e-mails), board_seats vs committee_seats, by_category, by_project, by_role, editable vs foundation_controlled; include_seats adds the rows (name, e-mail, role, voting status, committee, project). " +
			"category keeps one committee category, matched case-insensitively. The caller needs the organization grant (auditor or writer) LFX Self Serve requires; the result is complete for the scope, never truncated, large foundations being read in several requests. Seats only; the membership's contact of record is get_membership_key_contacts and can be a different person. " +
			"include_membership_contacts=true adds membership_contacts (the organization's key contacts on the projects in scope: contact of record per membership, role and status as stored) and representation (per project: the Representative/Voting Contact rows beside the seats that represent it: board, or Voting Rep / Alternate Voting Rep on any committee): two answers to who represents the organization, labelled, never merged; by_voting_status is always returned.",
