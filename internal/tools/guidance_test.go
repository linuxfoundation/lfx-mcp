...
func TestSemanticLayerGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"as the\nfoundation slug (cncf, not k8s)",
		"subprojects=separate is rejected since they have no by=project",
...
func TestStandardMetricsGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"combined and excluded are NO-OPS, and subprojects=separate\nis REJECTED like any separate without by=project",
